...
serve: ## Serve Quartz locally
	...
	hugo server --enableGitInfo --minify --bind=$(or $(HUGO_BIND),0.0.0.0) \
		--baseURL=$(or $(HUGO_BASEURL),http://localhost) \
		--port=$(or $(HUGO_PORT),8313) \
		--appendPort=$(or $(HUGO_APPENDPORT),true)
